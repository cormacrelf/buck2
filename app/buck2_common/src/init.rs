...
impl HttpConfig {
    pub fn from_config(config: &LegacyBuckConfig) -> buck2_error::Result<Self> {
        let connect_timeout_ms =
            config.parse(BuckconfigKeyRef::new("http", "connect_timeout_ms"))?;
        ...
        let max_concurrent_requests =
            config.parse(BuckconfigKeyRef::new("http", "max_concurrent_requests"))?;
...
impl SystemWarningConfig {
    pub fn from_config(config: &LegacyBuckConfig) -> buck2_error::Result<Self> {
        let memory_pressure_threshold_percent = config.parse(BuckconfigKeyRef::new(
            "buck2_system_warning",
            "memory_pressure_threshold_percent",
        ))?;
        let remaining_disk_space_threshold_gb = config.parse(BuckconfigKeyRef::new(
            "buck2_system_warning",
            "remaining_disk_space_threshold_gb",
        ))?;
        let min_re_download_bytes_threshold = config.parse(BuckconfigKeyRef::new(
            "buck2_system_warning",
            "min_re_download_bytes_threshold",
        ))?;
        let avg_re_download_bytes_per_sec_threshold = config.parse(BuckconfigKeyRef::new(
            "buck2_system_warning",
            "avg_re_download_bytes_per_sec_threshold",
        ))?;
        let optin_vpn_check_targets_regex = config.parse(BuckconfigKeyRef::new(
            "buck2_health_check",
            "optin_vpn_check_targets_regex",
        ))?;
        let enable_stable_revision_check = config.parse(BuckconfigKeyRef::new(
            "buck2_health_check",
            "enable_stable_revision_check",
        ))?;
        let enable_health_check_process_isolation = config.parse(BuckconfigKeyRef::new(
            "buck2_health_check",
    ...
    pub fn from_config(config: &LegacyBuckConfig) -> buck2_error::Result<Self> {
        if let Some(env_conf) = buck2_env!(
            ...
            let status_if_min_daemon_cgroup_version: Option<u32> =
                config.parse(BuckconfigKeyRef::new(
                    "buck2_resource_control",
                    "status_if_min_daemon_cgroup_version",
                ))?;
            ...
            let memory_max = config.parse(BuckconfigKeyRef::new(
                "buck2_resource_control",
                "memory_max",
            ))?;
            let memory_high = config.parse(BuckconfigKeyRef::new(
                "buck2_resource_control",
                "memory_high",
            ))?;
            let memory_max_per_action = config.parse(BuckconfigKeyRef::new(
                "buck2_resource_control",
                "memory_max_per_action",
            ))?;
            let memory_high_per_action = config.parse(BuckconfigKeyRef::new(
                "buck2_resource_control",
                "memory_high_per_action",
            ))?;
            let memory_high_actions = config.parse(BuckconfigKeyRef::new(
                "buck2_resource_control",
                "memory_high_actions",
            ))?;
            let memory_max_actions = config.parse(BuckconfigKeyRef::new(
                "buck2_resource_control",
                "memory_max_actions",
            ))?;
            let enable_suspension = config.parse(BuckconfigKeyRef::new(
                "buck2_resource_control",
                "enable_suspension",
            ))?;
            ...
            let experimental_suspension_algo_variant = config.parse(BuckconfigKeyRef::new(
                "buck2_resource_control",
...
impl HealthCheckConfig {
    pub fn from_config(config: &LegacyBuckConfig) -> buck2_error::Result<Self> {
        let enable_health_checks = config.parse(BuckconfigKeyRef::new(
            "buck2_health_check",
            "enable_health_checks",
        ))?;
        let disabled_health_check_names = config.parse(BuckconfigKeyRef::new(
            "buck2_health_check",
...
impl DaemonStartupConfig {
    pub fn new(config: &LegacyBuckConfig) -> buck2_error::Result<Self> {
        ...
        Ok(Self {
            ...
            daemon_idle_timeout_s: config
                .parse(BuckconfigKeyRef::new("buck2", "daemon_idle_timeout_s"))?,
        })
    }
