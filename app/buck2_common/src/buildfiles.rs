...
pub fn parse_buildfile_name(
    ...
    } else if let Some(buildfiles_value) =
        config.parse_list::<String>(BuckconfigKeyRef::new("buildfile", "name"))?
    {
    ...
    if let Some(buildfile) =
        config.parse::<String>(BuckconfigKeyRef::new("buildfile", "extra_for_test"))?
    {
