    ...
    async fn init_data(
        ...
        let init_fut = async move {
            ...
            let retry_backoff = Duration::from_millis(
                root_config
                    .parse(BuckconfigKeyRef::new("buck2", "event_log_retry_backoff_duration_ms"))?
                    .unwrap_or(500),
            );
            let retry_attempts = root_config
                .parse(BuckconfigKeyRef::new("buck2", "event_log_retry_attempts"))?
                .unwrap_or(5);
            let message_batch_size = root_config.parse(BuckconfigKeyRef::new("buck2", "event_log_message_batch_size"))?;
            ...
            let deferred_materializer_configs = {
                let defer_write_actions = root_config
                    .parse::<RolloutPercentage>(BuckconfigKeyRef::new("buck2", "defer_write_actions"))?
                ...
                let ttl_refresh_frequency = root_config
                    .parse(BuckconfigKeyRef::new("buck2", "ttl_refresh_frequency_seconds"))?
                    .unwrap_or(1800);

                let ttl_refresh_min_ttl = root_config
                    .parse(BuckconfigKeyRef::new("buck2", "ttl_refresh_min_ttl_seconds"))?
                    .unwrap_or(3600);

                let ttl_refresh_enabled = root_config
                    .parse::<RolloutPercentage>(BuckconfigKeyRef::new("buck2", "ttl_refresh_enabled"))?
                    .unwrap_or_else(RolloutPercentage::never)
                    .roll();
                ...
                let verbose_materializer_log = root_config
                    .parse(BuckconfigKeyRef::new("buck2", "verbose_materializer_event_log"))?
                    .unwrap_or(false);

                let clean_stale_config = CleanStaleConfig::from_buck_config(root_config)?;

                let disable_eager_write_dispatch = root_config
                    .parse::<RolloutPercentage>(BuckconfigKeyRef::new("buck2", "disable_eager_write_dispatch"))?
                    .unwrap_or_else(RolloutPercentage::never)
                    .roll();

                let eager_materialization_enabled = root_config
                    .parse::<RolloutPercentage>(BuckconfigKeyRef::new("buck2", "eager_materialization_enabled"))?
                    .unwrap_or_else(RolloutPercentage::never)
                    .roll();
            ...
            let use_network_action_output_cache = root_config
                .parse(BuckconfigKeyRef::new("buck2", "use_network_action_output_cache"))?
                .unwrap_or(false);
            ...
            let remote_dep_files_enabled = root_config
                .parse(BuckconfigKeyRef::new("build", "remote_dep_file_cache_enabled"))?
                .unwrap_or(false);
            ...
            let declare_output_has_content_based_path_default =
                root_config.parse(BuckconfigKeyRef::new("buck2", "declare_output_has_content_based_path_default"))?;
            init_declare_output_has_content_based_path_default(
                declare_output_has_content_based_path_default,
            )?;

            let action_has_content_based_path_default = root_config.parse(BuckconfigKeyRef::new("buck2", "action_has_content_based_path_default"))?;
