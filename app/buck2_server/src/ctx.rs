...
impl DiceCommandUpdater<'_, '_> {
    fn make_user_computation_data(
        ...
        if let Some(max_lines) =
            root_config.parse(BuckconfigKeyRef::new("ui", "thread_line_limit"))?
        {
            self.cmd_ctx
                .events()
        ...
        let persistent_worker_shutdown_timeout_s = root_config
            .parse::<u32>(BuckconfigKeyRef::new(
                "build",
        ...
        let re_cancel_on_estimated_queue_time_exceeds = root_config
            .parse::<u64>(BuckconfigKeyRef::new(
                "build",
                "remote_execution_cancel_on_estimated_queue_time_exceeds_s",
            ))?
            .map(Duration::from_secs);
        let re_fallback_on_estimated_queue_time_exceeds = root_config
            .parse::<u64>(BuckconfigKeyRef::new(
                "build",
                "remote_execution_fallback_on_estimated_queue_time_exceeds_s",
            ))?
            .map(Duration::from_secs);
        ...
        let output_trees_download_semaphore_size = root_config.parse::<u32>(
            BuckconfigKeyRef::new("buck2", "output_trees_download_semaphore_size"),
        )?;
        ...
        let override_use_case = root_config.parse::<RemoteExecutorUseCase>(
